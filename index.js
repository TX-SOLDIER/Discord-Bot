...
client.on('messageCreate', async (message) => {
  ...
  // ---- Question of the Day (QOTD) ----
    ...
    if (subcommand === 'on') {
      ...
      message.reply('✅ QOTD activated in this channel!');
      const interval = setInterval(() => {
        if (!qotdChannels.has(message.channel.id)) return clearInterval(interval);
        const question = spicyTruths[Math.floor(Math.random() * spicyTruths.length)];
        message.channel.send(`📌 **Question of the Day:** ${question}`);
      }, 24 * 60 * 60 * 1000); // Sends once every 24 hours
      qotdIntervals.set(message.channel.id, interval);
    } else if (subcommand === 'off') {
      if (!qotdChannels.has(message.channel.id)) return message.reply('❌ QOTD is not active in this channel.');
      ...
      message.reply('✅ QOTD deactivated in this channel.');
    } else {
      message.reply('❓ Usage: $qotd on/off');
    }
  }
