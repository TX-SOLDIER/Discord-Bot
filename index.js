...
if (!fs.existsSync(warnsFile)) fs.writeFileSync(warnsFile, '{}', 'utf-8');
let warns = JSON.parse(fs.readFileSync(warnsFile, 'utf-8'));
...
// ---- WARN SYSTEM ----
  ...
  warns[member.id].push({ reason, date: new Date().toISOString(), moderator: message.author.id });
  ...
  let warnList = userWarns.map((w, i) => `${i+1}. ${w.reason} (by <@${w.moderator}> on ${new Date(w.date).toLocaleString()})`).join('\n');
